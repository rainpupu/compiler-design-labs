...
function renderLR0(lr0, compact = false) {
  ...
  const lines = []
  lines.push(`增广开始符号：${lr0.augmentedStart} -> ${lr0.startSymbol}`)
  lines.push(`产生式数量：${lr0.productions.length}`)
  lines.push(`项目集状态数量：${lr0.states.length}`)
  lines.push(`GOTO 转移数量：${lr0.transitions.length}`)
  lines.push(`LR(0) 判定：${lr0.isLR0 ? '通过，无冲突' : '不通过，存在冲突'}`)
  lines.push('')
  lines.push('产生式编号：')
  lr0.productions.forEach((production) => lines.push(`(${production.index}) ${production.text}`))
  lines.push('')
  lines.push('项目集规范族：')
  ...
  states.forEach((state) => {
    ...
    const conflict = lr0.conflicts[state.id]
    if (conflict?.shiftReduce) lines.push(`  [移进-归约冲突] shift on: ${conflict.shiftSymbols.join(', ')}`)
    if (conflict?.reduceReduce) lines.push(`  [归约-归约冲突] ${conflict.reduceItems.join('; ')}`)
  })
...
function renderSLR1(slr) {
  ...
  if (slr.conflicts.length) {
    lines.push('冲突详情：')
    slr.conflicts.forEach((conflict) => lines.push(`I${conflict.stateId}, ${conflict.symbol}: ${conflict.type} => ${conflict.actions.join(' / ')}`))
    lines.push('')
  }
  lines.push('FOLLOW 集：')
...
function resultVerdict(semantic, slr) {
  ...
  if (semantic.errors.length) reasons.push(...semantic.errors.map((e) => `[${e.kind}] line ${e.line || '-'} ${e.lexeme ? `near "${e.lexeme}" ` : ''}${e.message}`))
...
function analyzeOne(file, grammarText, options = {}) {
  ...
  if (verdict.reasons.length) {
    lines.push('错误原因：')
    verdict.reasons.forEach((reason, index) => lines.push(`${index + 1}. ${reason}`))
  }
...
if (grammarArg) {
  const grammarPath = resolvePath(grammarArg)
  if (!grammarPath) { console.error(`找不到文法文件：${grammarArg}`); process.exit(1) }
  grammarText = fs.readFileSync(grammarPath, 'utf8')
}
...
if (inputArg) {
  ...
  try {
    const { report } = analyzeOne(file, grammarText, { compact: false })
    console.log(report)
  } catch (err) {
    console.error(runtimeReport(file, err))
    process.exit(2)
  }
  process.exit(0)
}
...
if (batchArg) {
  ...
  const outDirArg = getArg('--out-dir', '-d') || 'acceptance-output'
  ...
  for (const file of files) {
    ...
    const outputFile = path.join(outDir, `${base}.acceptance.txt`)
    try {

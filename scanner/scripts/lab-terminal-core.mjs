...
export function scanSource(code) {
  ...
  const push = (type, lexeme, start, end, tokenLine) => {
...
class SymbolTable {
  ...
  enter(name) {
    ...
    const scope = { id: this.scopes.length, name, parent: parent.id, level: parent.level + 1, map: new Map() }
  ...
  lookup(name) {
    for (let i = this.stack.length - 1; i >= 0; i -= 1) {
      const found = this.stack[i].map.get(name)
...
class Parser {
  ...
  emit(op, arg1 = '', arg2 = '', result = '') {
    this.quads.push({ index: this.quads.length, op, arg1: String(arg1 ?? ''), arg2: String(arg2 ?? ''), result: String(result ?? '') })
...
export function analyzeSource(source) {
  ...
  return {
    ...
    quads: parser.quads,
    steps: parser.steps,
    stats: { tokens: tokens.length, astNodes: nextAstId, symbols: parser.symbolTable.rows.length, errors: parser.errors.length, quads: parser.quads.length },
  }
}

...
export function allocateMemory(semanticResult) {
  ...
  for (const symbol of symbols) {
    ...
    const size = String(symbol.kind || '').includes('array') ? arraySize(symbol.extra, typeSize(symbol.type)) : typeSize(symbol.type)
    const address = DATA_BASE + dataOffset
    dataOffset += Math.max(size, 4)
    const row = {
      name: symbol.name,
      kind: symbol.kind,
      type: symbol.type,
  ...
  const addConst = (literal) => {
    ...
    if (!isNumberLiteral(text) || constAddress.has(text)) return
    const address = CONST_BASE + constOffset
    constOffset += 4
    constAddress.set(text, hex(address))
    constRows.push({ literal: text, size: 4, address: hex(address), region: 'CONST' })
  }
  ...
  quads.forEach((quad, index) => {
    ...
    ;[quad.arg1, quad.arg2, quad.result].forEach((operand) => {
      ...
      if (isNumberLiteral(operand)) addConst(operand)
    })
  })
...
export function generateTargetCode(semanticResult, memory = allocateMemory(semanticResult)) {
  ...
  for (const quad of semanticResult.quads || []) {
    ...
    switch (quad.op) {
      case 'func':
        lines.push(`${prefix}FUNC ${quad.result}`)
        break
      case 'end':
        lines.push(`${prefix}END ${quad.result}`)
        break
      case '=':
        lines.push(`${prefix}MOV ${resolved(memory, quad.result)}, ${resolved(memory, quad.arg1)}`)
        break
      case '+': case '-': case '*': case '/': case '<': case '<=': case '>': case '>=': case '==': case '!=': case '&&': case '||':
        lines.push(`${prefix}${binaryMnemonic(quad.op)} ${resolved(memory, quad.result)}, ${resolved(memory, quad.arg1)}, ${resolved(memory, quad.arg2)}`)
        break
      case 'uminus':
        lines.push(`${prefix}NEG ${resolved(memory, quad.result)}, ${resolved(memory, quad.arg1)}`)
        break
      case '!':
        lines.push(`${prefix}NOT ${resolved(memory, quad.result)}, ${resolved(memory, quad.arg1)}`)
        break
      case 'jz':
        lines.push(`${prefix}JZ ${resolved(memory, quad.arg1)}, ${resolved(memory, quad.result)}`)
        break
      case 'jmp':
        lines.push(`${prefix}JMP ${resolved(memory, quad.result)}`)
        break
      case 'label':
        lines.push(`${prefix}LABEL ${quad.result}`)
        break
      case 'arg':
        lines.push(`${prefix}ARG ${resolved(memory, quad.arg1)}`)
        break
      case 'call':
        lines.push(`${prefix}CALL ${quad.arg1}, ${quad.arg2}${quad.result ? `, ${resolved(memory, quad.result)}` : ''}`)
        break
      case 'return':
        lines.push(`${prefix}RET ${resolved(memory, quad.arg1)}`)
        break
      case 'print':
        lines.push(`${prefix}PRINT ${resolved(memory, quad.arg1)}`)
        break
      case 'input':
        lines.push(`${prefix}INPUT ${resolved(memory, quad.result)}`)
        break
      case '[]':
        lines.push(`${prefix}LOADIDX ${resolved(memory, quad.result)}, ${resolved(memory, quad.arg1)}, ${resolved(memory, quad.arg2)}`)
        break
      default:
        lines.push(`${prefix}${quad.op.toUpperCase()} ${[quad.arg1, quad.arg2, quad.result].filter(Boolean).map((x) => resolved(memory, x)).join(', ')}`)
    }
  }
  return lines
}
...
export function memoryStats(memory) {
  return {
    dataCount: memory.symbols.length,
    tempCount: memory.temporaries.length,
    constCount: memory.constants.length,
    codeCount: memory.code.length,
